...
$bbox[0] = "&BBox=".($cooCentroX-($ancho*$zoom)).",".($cooCentroY-($alto*$zoom/2)).",".$cooCentroX.",".($cooCentroY+($alto*$zoom/2));
$bbox[1] = "&BBox=".$cooCentroX.",".($cooCentroY-($alto*$zoom/2)).",".($cooCentroX+($ancho*$zoom)).",".($cooCentroY+($alto*$zoom/2));

getWMSParams($capas);

$imgWest = "urlService=".$servidorWMS[0]."&SERVICES=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=".$layer[0]."&FORMAT=image/jpeg&CRS=EPSG:3857".$bbox[0]."&WIDTH=".$ancho."&HEIGHT=".$alto;
$imgEast = "urlService=".$servidorWMS[1]."&SERVICES=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=".$layer[1]."&FORMAT=image/jpeg&CRS=EPSG:3857".$bbox[1]."&WIDTH=".$ancho."&HEIGHT=".$alto;
